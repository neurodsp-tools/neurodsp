...
def test_sim_combined():
    ...
    components = {'sim_oscillation' : {'freq' : FREQ1},
                   'sim_powerlaw' : {'exponent' : -2}}

    out = sim_combined(N_SECONDS, FS, components)
    check_sim_output(out)

    # Test case with multiple uses of same function
    components = {'sim_oscillation' : [{'freq' : FREQ1}, {'freq' : FREQ2}],
                  'sim_powerlaw' : {'exponent' : -2}}
    variances = [0.5, 0.5, 1]
    out = sim_combined(N_SECONDS, FS, components, variances)
    check_sim_output(out)
...
def test_sim_peak_oscillation():
    ...
    sig = sim_peak_oscillation(sig_ap, FS, FREQ1, bw=4, height=2)
...
def test_sim_combined_peak():

    components = {'sim_powerlaw' : {'exponent' : -2},
                  'sim_peak_oscillation' : {'freq' : FREQ1, 'bw' : 4, 'height' : 2}}

    out = sim_combined_peak(N_SECONDS, FS, components)
    check_sim_output(out)
